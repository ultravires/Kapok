...
void TitleBar::initControl() {
    ...
    m_pButtonMax->setIcon( QIcon( ":/icons/window/maximize_white.png" ) );
    m_pButtonMax->setIconSize( QSize( BUTTON_WIDTH, BUTTON_WIDTH ) );
    m_pButtonClose->setIcon( QIcon( ":/icons/window/close_white.png" ) );
    m_pButtonClose->setIconSize( QSize( BUTTON_WIDTH, BUTTON_WIDTH ) );

    m_pButtonMin->setFixedSize( QSize( BUTTON_WIDTH, BUTTON_WIDTH ) );
    ...
    mylayout->setContentsMargins( 15, 0, 15, 0 );
    mylayout->setSpacing( 0 );

    m_pTitleContent->setSizePolicy( QSizePolicy::Expanding,
                                    QSizePolicy::Fixed );
...
// 设置标题栏长度;
void TitleBar::setTitleWidth( int width ) { this->setFixedWidth( width ); }
...
void TitleBar::setTitleRoll() {
    connect( &m_titleRollTimer, SIGNAL( timeout() ), this,
             SLOT( onRollTitle() ) );
...
void TitleBar::paintEvent( QPaintEvent *event ) {
    ...
    if ( !m_isTransparent ) {
        ...
        pathBack.addRoundedRect( QRect( 0, 0, this->width(), this->height() ),
                                 3, 3 );
        painter.setRenderHint( QPainter::Antialiasing, true );
        painter.fillPath( pathBack,
                          QBrush( QColor( m_colorR, m_colorG, m_colorB ) ) );
    }
    ...
    if ( this->width() !=
         ( this->parentWidget()->width() - m_windowBorderWidth ) ) {
...
void TitleBar::onButtonCloseClicked() { emit signalButtonCloseClicked(); }
